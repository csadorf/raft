  ...
  const raft::device_mdspan<ElementType, Extents, LayoutPolicy, AccessorPolicy>& obj)
{
  ...
  if (obj.size() > 0) {
    ...
    if (of.good()) {
      std::cout << "Written " << obj.size() * sizeof(obj.data_handle()[0]) << " bytes" << std::endl;
    } else {
                 ...
                 raft::device_mdspan<ElementType, Extents, LayoutPolicy, AccessorPolicy>& obj)
{
  ...
  if (obj.size() > 0) {
    for (int i = 0; i < (int)obj.rank(); i++) {
      ...
      if (obj.extent(i) != ex) {
        std::cerr << "Incorrect extent while reading mdarray " << ex << " vs. " << obj.extent(i)
                  << " at " << i << std::endl;
      }
    }
    cudaStream_t stream = handle.get_stream();
    ...
    handle.sync_stream(stream);
    if (file.good()) {
      std::cout << "read " << obj.size() * sizeof(obj.data_handle()[0]) << " bytes" << std::endl;
    } else {
